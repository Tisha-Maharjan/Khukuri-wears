...
if(isset($_POST['add_to_cart'])){
  ...
  $check_cart_numbers = mysqli_query($conn, "SELECT * FROM `cart` WHERE title = '$product_name' AND user_id = '$user_id' AND size = '$size'") or die('query failed');
  ...
  if(mysqli_num_rows($check_cart_numbers) > 0){
    ?>
      ...
      <?php
      $cart_query = mysqli_query($conn, "SELECT COUNT(*) AS cart_count FROM cart WHERE user_id = '$user_id'");
      $cart_data = mysqli_fetch_assoc($cart_query);
      $cart_num = $cart_data['cart_count'];

      // Update the cart count displayed in the HTML
      echo "<script>document.getElementById('cart-count').innerHTML = $cart_num;</script>";
  }
}
