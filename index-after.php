...
?>
      ...
      if($image_name==""){
        ...
        ?>
          ...
          <a href="products-after.php?category_title=<?php echo $title; ?>"> <img
    ...
    <section class="section-contact" id="contact">
      <div class="contact-container">
        ...
        <div class="contact-form">
          ...
          <button type="submit" name="submit" class="contact-submit">Submit</button>
          </div>
  </form>
  ...
  if(isset($_POST['submit'])){
    ...
    $check_msg=mysqli_query($conn, "SELECT * FROM contact WHERE name='$name'AND email='$email' AND  message='$message' ") or die('query failed');
  if(mysqli_num_rows($check_msg)>0){
    ?>
    <script>
      swal("Message already sent!");
    </script>
    <?php
    // echo "<script>
    //     alert('Message already submitted.'); 
    //   </script>";
  }else{
    $sql="INSERT INTO contact(name, email, message)
        VALUES('$name', '$email', '$message')";
    $result=mysqli_query($conn, $sql) or die("Data is not inserted due to ".mysqli_error($conn));
